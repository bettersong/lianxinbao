...
<?php include APP_PATH."/application/views/header".$_m.".php"; //包含公共的头部 ?>
  <!-- <?php var_dump($_SESSION['departmentInfo']); ?> -->
   ...
   $("td.disabled").click(function(e) {
    ...
    $(".update").click(function () {
      ...
      $.ajax({
        ...
        type: "POST",
        dataType: 'json',
        url: "<?=CURRENT_DIR?>/userAmin_update.php",
    ...
    $(".reset").click(function () {
      ...
      var id = $.trim( $(this).parent("td").parents("tr").eq(0).find("td").eq(0).attr("ids"));
      var password = $.trim( $(this).parents("tr").eq(0).find("td").eq(2).find("input").eq(0).val());
      alert(id+"*****"+password);
      $.ajax({
        async: false,
        data: {
        ...
        type: "POST",
        dataType: 'json',
        url: "<?=CURRENT_DIR?>/index_reset.php",
